...
database.connect((err) => {
  if (err) throw err;
  console.log("API execute report user successfully connected to the server");
});
  ...
  try {
    ...
    switch (table) {
      case 1:
        ...
        break;
      case 1:
        tablename = "ReportProduct"
        break;
      case 1:
        tablename = "ReportPost"
        break;
      default:
        break;
    }
          ...
          if (starttime < endtime && endtime > starttime) {
            ...
            if (users.penalty === null) {
              if (users.userid !== response.data.userid) {
                ...
                const success_commitReport = await EditReportData(reportid, tablename);
                if (success_createpenalt && success_addpenalt && success_commitReport) {
              ...
              if (penalty !== null) {
                if (penalty.endtime >= Date().now) {
                  ...
                  const success_commitReport = await EditReportData(reportid, tablename);
                  if (success_commitReport) {
                    res.status(200).json({ status: true, message: 'Người dùng này đã có lệnh trường phạt rồi!' });
                  }
                  else {
                    res.status(200).json({ status: false, message: 'Lỗi khi trừng phạt!' });
                  }
...
async function EditReportData(reportid, table) {
  try {;
    const result = await database.query(`UPDATE ${table} SET complete = true WHERE reportid = ?`, 
      [ reportid ]);
      return true;
  }
  catch (e) {
