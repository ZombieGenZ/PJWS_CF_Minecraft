...
database.connect((err) => {
  if (err) throw err;
  console.log("API delete report user successfully connected to the server");
});
...
routes.post("/", async (req, res) => {
  ...
  async function GetReportData(reportid) {
    return new Promise((resolve, reject) => {
      database.query(`SELECT * FROM ReportUser WHERE reportid = ?`, [reportid], (err, res) => {
        if (err) {
          reject(err);
        } else {
          if (res.length > 0) {
  ...
  async function DeleteReport(reportid) {
    try {
      const result = await database.query(`DELETE FROM ReportUser WHERE reportid = ?`, 
        [ evaluateid ]);
        return true;
    }
    catch (e) {
